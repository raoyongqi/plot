...
x = tf.keras.layers.Conv1D(filters=32, kernel_size=1)(x) # create 32 values so we will not have to broadcast in the first layer
num_blocks = 3
...
model.save('residualdilated3blocks.h5')
...
def evaluation(X_valid, y_valid):
    ...
    results_df.to_csv("residual_dilated3_actual_vs_predicted.csv", index=False)

    # 获取训练历史并保存
    train_df = pd.DataFrame(history.history)
    train_df.to_csv("residual_dilated3_training_history.csv", index=False)
