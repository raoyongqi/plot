...
model.save('residualdilated20blocks.h5')
...
def evaluation(X_valid, y_valid):
    ...
    results_df.to_csv("residual_dilated20_actual_vs_predicted.csv", index=False)

    # 获取训练历史并保存
    train_df = pd.DataFrame(history.history)
    train_df.to_csv("residual_dilated20_training_history.csv", index=False)
